...
tic
orbInc=  98.2056;
meanMo = 14.63691895;%revolutions per day
SGPdragp1 = 3.6800e-06;  % from ---First time derivative of mean motion(rad/min^2)
SGPdragp2 =0;     % Second time derivative of mean motion (rad/min^3
SGP4dragp = 0.7053; %Bstar
n0 = 2*pi*meanMo/1440; % Mean motion (rad/min) 
    e0 = 0.0028; % Eccentricity (0.0<=e0>=1.0)   2)
    i0 = pi*orbInc/180; % Inclination (rad)      3)
    M0 = 9.4278*pi/180; %5.339; % Mean anomaly (rad)    4)
    w0 = 267.5222*pi/180; %054.5309; % Argument of perigee (rad)   5)
    Ohm0 = 329.2442*pi/180; %rsc
    dn0 = 2*2*pi*SGPdragp1/(1440^2); % First time derivative of mean motion(rad/min^2)
    ddn0 = 6*2*pi*SGPdragp2/(1440^3); % Second time derivative of mean motion (rad/min^3)
    Bstar = SGP4dragp; % SGP4 type drag coefficient
    t0 =0;
